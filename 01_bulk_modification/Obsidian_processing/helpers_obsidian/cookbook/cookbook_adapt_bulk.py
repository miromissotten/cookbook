...
def rename_markdown_subheader(cookbook_path, text_dir_path, pages_index_path, old_title, new_title):
    """
    Finds a specific subheader (e.g., '### Ingredients:') and renames it 
    to a new one (e.g., '### Ingredients') across all referenced files.
    """
    logger = get_logger()
    old_header = f"### {old_title}"
    new_header = f"### {new_title}"

    logger.info(f"--- Renaming Header: '{old_title}' -> '{new_title}' ---")
    ...
    for filename, file_path in iter_md_files(text_dir_path, referenced_files):
        ...
        for line in lines:
            if line.strip() == old_header:
                newline_char = "\n" if line.endswith("\n") else ""
                new_lines.append(new_header + newline_char)
                file_changed = True
            else:
        ...
        if file_changed:
            ...
            logger.info(f"  -> Updated header in '{filename}'")
            count_updated += 1

    logger.info(f"\nFinished! Updated {count_updated} files.")
